...
def notification_handler(sender, data):
    ...
    if hundredPercent%10 == 0 :
        print(" %",hundredPercent," downloaded")
    #print(lists)
    if len(allNotifiy) ==  int.from_bytes(bytearray(allNotifiy[0])[4:8],"little"):
        print("************** " ,len(allNotifiy) ," Total Counter   , Our Target" ,int.from_bytes(bytearray(allNotifiy[0])[4:8] , "little") )
        print("Download is successfully")


def notificationHandlerResult(sender, data):
    print("X : "  , s16floatfactor(data[0:2] , 1 ) , " Y : " , s16floatfactor(data[2:4] , 1 ) , "Z :" , s16floatfactor(data[4:6] , 1 )  )
    #print(data)
